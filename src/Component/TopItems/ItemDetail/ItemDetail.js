...
const ItemDetail = () => {
   ...
   useEffect(() => {
      fetch(`https://morning-retreat-07507.herokuapp.com/product/${params.id}`)
         .then((res) => res.json())
         .then((data) => {
            setItem(data);
   ...
   const handleQuantity = () => {
      if (quantityNumber > 0) {
         fetch(
            `https://morning-retreat-07507.herokuapp.com/updateQuantity/${params.id}`,
            {
               method: "PUT",
               headers: {
   ...
   const restockSubmit = (event) => {
      ...
      fetch(
         `https://morning-retreat-07507.herokuapp.com/restockQuantity/${params.id}`,
         {
            method: "PUT",
            headers: {
               "content-type": "application/json",
